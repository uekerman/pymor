...
import cppyy
from cppyy.gbl.std import *
demo_path = Path('./minimal_cpp_demo_cppyy').resolve().absolute()
cppyy.add_include_path(str(demo_path))
cppyy.cppdef((demo_path/'model.cc').read_text())
```
...
```{code-cell} ipython3
mymodel = cppyy.gbl.DiffusionOperator(10, 0, 1)
myvector = cppyy.gbl.Vector(10, 0)
...
import numpy as np
import math
from cppyy.gbl import Vector, DiffusionOperator
